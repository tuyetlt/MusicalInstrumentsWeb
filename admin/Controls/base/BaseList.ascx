<%@ Control Language="C#" AutoEventWireup="true" CodeFile="BaseList.ascx.cs" Inherits="admin_Controls_base_BaseList" %>
...
<div class="obj-list">
    <div class="filter">
        ...
        <input type="text" id="categoryid" name="categoryidlist" style="display: none" />
        <input type="text" id="categoryname" name="categorynamelist" style="display: none" />
        <select id="drCategory" data-level="1" name="itemSelect"></select>

        <input type="hidden" id="Flag" value="0" />
        <select id="flags">
            <option value="0">Tùy chọn lọc</option>
            <option value="<%= (int)BaseTableFlag.Social %>">Mạng xã hội</option>
            <option value="<%= (int)BaseTableFlag.Partner %>">Đối tác</option>
            <option value="<%= (int)BaseTableFlag.Support %>">Nick hỗ trợ</option>
        </select>
