...
public partial class admin_Controls_home_partner_PartnerUpdate : System.Web.UI.UserControl
{
    #region Variable
    public DataRow dr;
    ...
    int ID = 0, IDCopy = 0;
    public string click_action, control, table = "tblHome_Partner";
    ...
    protected void UpdateDatabase()
    {
        if (!String.IsNullOrEmpty(click_action) && (click_action == "save" || click_action == "saveandback" || click_action == "saveandcopy" || click_action == "saveandadd"))
        {
            ...
            using (var db = MetaNET.DataHelper.SqlService.GetSqlService())
            {
                ...
                if (IsUpdate)
                    sqlQuery = @"UPDATE[dbo].[tblHome_Partner] SET [ID]=@ID,[Name]=@Name,[Description]=@Description,[Sort]=@Sort,[Flags]=@Flags,[Hide]=@Hide,[Image_1]=@Image_1,[Alt]=@Alt,[CreatedDate]=@CreatedDate,[EditedDate]=@EditedDate,[CreatedBy]=@CreatedBy,[EditedBy]=@EditedBy WHERE [ID] = @ID";
                else
                    sqlQuery = @"INSERT INTO [dbo].[tblHome_Partner]([Name],[Description],[Sort],[Flags],[Hide],[Image_1],[Alt],[CreatedDate],[EditedDate],[CreatedBy],[EditedBy]) OUTPUT INSERTED.ID VALUES (@Name,@Description,@Sort,@Flags,@Hide,@Image_1,@Alt,@CreatedDate,@EditedDate,@CreatedBy,@EditedBy)";
